...
def sessions_list(request, page_number):
    if request.method == 'GET':
        sessions = EnteteSession.objects.all()[
                   (int(page_number) - 1) * page_size:(int(page_number) - 1) * page_size + page_size]
...
def transfert_optimise(request):
    ...
    # === BULK FETCH STOCKS and VENTES ===
    stocks = Stock.objects.filter(
        code_article_dem__in=article_codes,
        code_etab__in=all_etabs
    ).values(
        'code_etab', 'code_article_dem', 'stock_physique', 'code_depot', 'code_barre', 'ventes'
    )
    stock_lookup = {}
    ventes_lookup = {}
    for s in stocks:
        key = (s['code_etab'], s['code_article_dem'])
        stock_lookup[key] = s
        ventes_lookup[key] = s.get('ventes', 0)
    ...
    for art in articles:
        ...
        if critere == 'vider':
            ...
            for emetteur in emetteurs_with_stock:
                ...
                if not eligible_recepteurs:
                    print(f"    [SENDER] No eligible recepteurs found for {emetteur}")
                    continue

                # Send one by one to eligible recepteurs (cycling through them)
                recepteur_index = 0
                while remaining_to_send > 0:
                    ...
                    # Cycle to next recepteur (allows giving multiple articles to same recepteur)
                    recepteur_index = (recepteur_index + 1) % len(eligible_recepteurs)
    ...
    if request.method == 'POST':
        ...
        if crit == "exist":
            ...
            for i, article in enumerate(articles):
                ...
                tmp = tmp.replace(" ", "")
                articles[
                    i] = f"{tmp_code_article}            {tmp[len(tmp_code_article):(len(tmp_code_article) + 6)]}         {tmp[len(tmp) - 1]}"
                article = f"{tmp_code_article}            {tmp[len(tmp_code_article):(len(tmp_code_article) + 6)]}         {tmp[len(tmp) - 1]}"
                if listes_article_couleur.get(
                        tmp_code_article + tmp[(len(tmp_code_article) + 3):(len(tmp_code_article) + 6)]) == None:
                    listes_article_couleur[
                        tmp_code_article + tmp[(len(tmp_code_article) + 3):(len(tmp_code_article) + 6)]] = [article]
                else:
                    listes_article_couleur[
                        tmp_code_article + tmp[len(tmp_code_article) + 3:len(tmp_code_article) + 6]] = \
                    listes_article_couleur[
                        tmp_code_article + tmp[len(tmp_code_article) + 3:len(tmp_code_article) + 6]] + [article]
                    ...
                    while offre and demande:
                        ...
                        if offre[i][8] > demande[i][8]:
                            ...
                            prop = Proposition(code_detaille_emet=id_emet.id_detaille,
                                               ...
                                               statut="en cours", etat="non modifier", stock_recep_sera=demande[i][5],
                                               stock_emet_sera=offre[i][5]
                                               , stock_recep_sera_couleur=demande[i][7][1],
                                               stock_emet_sera_couleur=offre[i][7][1])
        ...
        if crit == "articles_dem":
            ...
            for i, details in enumerate(results):
                ##print(details)
                details_instance = DetailleSession(code_session=id_s, code_article_dem=details[1], code_etab=details[2],
                                                   stock_physique=details[5], stock_min=details[6])
                    ...
                    for code_article in articles:
                        ...
                        while offre and demande:
                            ...
                            if offre[i][8] > demande[i][8]:
                                ...
                                prop = Proposition(code_detaille_emet=id_emet.id_detaille,
                                                   code_detaille_recep=id_recep.id_detaille, qte_trf=qte,
                                                   statut="en cours", etat="non modifier")
                                offre[i][8] = offre[i][8] - demande[i][8]
                                del demande[i]
                            elif offre[i][8] < demande[i][8]:
                                qte = offre[i][8]
                        ...
                        while offre and demande:
                            if offre[0][7] <= 0 and k != 0:
                                ...
                                id_recep = DetailleSession.objects.get(code_article_dem=demande[cpt_demande][1],
                                                                       code_etab=demande[cpt_demande][2],
                                                                       code_session=id_s)
                                prop = Proposition(code_detaille_emet=id_emet.id_detaille,
                                                   code_detaille_recep=id_recep.id_detaille, qte_trf=qte_a_trf,
                                                   statut="en cours",
